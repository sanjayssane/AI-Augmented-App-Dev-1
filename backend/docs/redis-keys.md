...
|---|---|---|---|
| `session:{token}` | Server-side auth session (HttpOnly cookie → token) | JSON: `user_id`, `role`, `created_at`, `last_seen_at`, `expires_at`, `csrf_token` | Examinee: 60 min inactivity; Examiner: 4 h inactivity (PRD §3.1.4) |
